...
void handle_login_user(std::stringstream &buffer, Address &addr_from,
  ...
  try {
    ...
    state.cdebug << userTag(packet.user_id) << "Asked to login" << std::endl;
    ...
    state.cdebug << userTag(packet.user_id) << "User already logged in"
                 << std::endl;
    response.status = ReplyLoginClientbound::ERR;
  } catch (UserNotRegisteredException &e) {
    state.cdebug << userTag(packet.user_id) << "User not registered"
                 << std::endl;
    ...
    state.cdebug << userTag(packet.user_id) << "Invalid user password"
                 << std::endl;
    response.status = ReplyLoginClientbound::ERR;
  } catch (FileOpenException &e) {
    state.cdebug << userTag(packet.user_id) << "Failed to open file"
                 << std::endl;
    response.status = ReplyLoginClientbound::ERR;
  } catch (FileWriteException &e) {
    state.cdebug << userTag(packet.user_id) << "Failed to write to file"
                 << std::endl;
    response.status = ReplyLoginClientbound::ERR;
  } catch (FileReadException &e) {
    state.cdebug << userTag(packet.user_id) << "Failed to read from file"
                 << std::endl;
    response.status = ReplyLoginClientbound::ERR;
...
void handle_logout_user(std::stringstream &buffer, Address &addr_from,
  ...
  try {
    packet.deserialize(buffer);
    state.cdebug << userTag(packet.user_id) << "Asked to logout user"
                 << std::endl;
    ...
    state.cdebug << userTag(packet.user_id) << "User not logged in"
                 << std::endl;
  } catch (WrongPasswordException &e) {
    state.cdebug << userTag(packet.user_id) << "Wrong password" << std::endl;
...

{
  ...
  try {
    ...
    state.cdebug << userTag(packet.user_id) << "User has no auctions"
                 << std::endl;
  } catch (FileOpenException &e) {
    state.cdebug << userTag(packet.user_id) << "Failed to open file"
                 << std::endl;
    ...
    std::cerr
        << "[ListMyAuctions] There was an unhandled exception that prevented "
...
void handle_list_auctions(std::stringstream &buffer, Address &addr_from,
  ...
  try {
    packet.deserialize(buffer);
    state.cdebug << "Asked to list auctions" << std::endl;

    std::vector<std::pair<uint32_t, bool>> auctions =
        state.file_manager.getAllAuctions();
    ...
    std::cerr
        << "[ListAuctions] There was an unhandled exception that prevented "
           "the server from listing auctions:"
        << e.what() << std::endl;
    return;
  }
...
void handle_show_record(std::stringstream &buffer, Address &addr_from,
  ...
  try {
    ...
    state.cdebug << auctionTag(packet.auction_id) << "Auction does not exist"
                 << std::endl;
    response.status = ReplyShowRecordClientbound::NOK;
  } catch (FileOpenException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Failed to open file"
                 << std::endl;
    response.status = ReplyShowRecordClientbound::ERR;
  } catch (FileWriteException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Failed to write to file"
                 << std::endl;
    response.status = ReplyShowRecordClientbound::ERR;
  } catch (FileReadException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Failed to read from file"
                 << std::endl;
    response.status = ReplyShowRecordClientbound::ERR;
  } catch (std::exception &e) {
...
void handle_open_auction(int connection_fd, AuctionServerState &state) {
  ...
  try {
    ...
    AuctionData auction(state.auctionsCount++, packet.user_id,
                        packet.auction_name, packet.start_value,
                        packet.time_active, packet.file_name, now, endTime, 0,
                        std::vector<Bid>());
    ...
    state.cdebug << userTag(packet.user_id) << " Invalid auction id"
                 << std::endl;
    response.status = ReplyOpenAuctionClientbound::NOK;
  } catch (InvalidAuctionNameException &e) {
    state.cdebug << userTag(packet.user_id) << " Invalid auction item name"
                 << std::endl;
    response.status = ReplyOpenAuctionClientbound::NOK;
  } catch (InvalidAuctionInitialBidException &e) {
    state.cdebug << userTag(packet.user_id) << " Invalid auction initial bid"
                 << std::endl;
    response.status = ReplyOpenAuctionClientbound::NOK;
  } catch (InvalidAuctionDurationException &e) {
    state.cdebug << userTag(packet.user_id) << " Invalid auction duration"
                 << std::endl;
    response.status = ReplyOpenAuctionClientbound::NOK;
  } catch (InvalidAuctionAssetException &e) {
    state.cdebug << userTag(packet.user_id)
                 << " Invalid auction asset fname or size" << std::endl;
    response.status = ReplyOpenAuctionClientbound::NOK;
  } catch (FileOpenException &e) {
    state.cdebug << userTag(packet.user_id) << " Failed to open file"
                 << std::endl;
...
void handle_close_auction(int connection_fd, AuctionServerState &state) {
  ...
  try {
    ...
    state.cdebug << "[" << auctionTag(packet.auction_id) << "]"
                 << " Auction closed" << std::endl;
  } catch (UserNotLoggedInException &e) {
    ...
    state.cdebug << auctionTag(packet.auction_id)
                 << " Auction not owned by user" << std::endl;
    response.status = ReplyCloseAuctionClientbound::EOW;
  } catch (AuctionNotActiveException &e) {
    state.cdebug << auctionTag(packet.auction_id) << " Auction already closed"
                 << std::endl;
    response.status = ReplyCloseAuctionClientbound::EOW;
  } catch (FileOpenException &e) {
    state.cdebug << userTag(packet.user_id) << " Failed to open file"
...
void handle_show_asset(int connection_fd, AuctionServerState &state) {
  ...
  try {
    ...
    state.cdebug << auctionTag(packet.auction_id) << "Asked to show asset"
                 << std::endl;
  } catch (AuctionDoesNotExistException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Auction does not exist"
                 << std::endl;
    ...
    state.cdebug << auctionTag(packet.auction_id) << "Invalid file path"
                 << std::endl;
    response.status = ReplyShowAssetClientbound::NOK;
  } catch (AssetDoesNotExistException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Asset does not exist"
                 << std::endl;
    response.status = ReplyShowAssetClientbound::NOK;
  } catch (FileOpenException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Failed to open file"
...
void handle_bid(int connection_fd, AuctionServerState &state) {
  ...
  try {
    ...
    state.cdebug << auctionTag(packet.auction_id) << "Auction not active"
                 << std::endl;
    response.status = ReplyBidClientbound::NOK;
  }

  catch (BidValueException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Bid value too low"
                 << std::endl;
    response.status = ReplyBidClientbound::NOK;
  } catch (UserIsOwnerException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Cannot bid on own auction"
                 << std::endl;
    response.status = ReplyBidClientbound::ILG;
  } catch (LargerBidAlreadyExistsException &e) {
    state.cdebug << auctionTag(packet.auction_id) << "Larger bid already exists"
                 << std::endl;
