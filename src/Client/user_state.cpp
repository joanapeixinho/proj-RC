...
bool UserState::isLoggedIn() {
  return this->user_id != NO_USER_ID;
}
...
void UserState::resolveServerAddress(std::string &hostname,
                                       std::string &port) {
...
void UserState::sendUdpPacketAndWaitForReply(UdpPacket &out_packet,
                                               UdpPacket &in_packet) {
